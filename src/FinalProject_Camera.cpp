...
int main(int argc, const char *argv[])
{
    ...
    /*
    Here Only 3 Combinations of descriptors and detectors are used for evaluation purpose.
    From Mid term project implementation, th etop 3 combinations are selected here. 
    */
   
    vector<string> detectorTypes   = {"FAST"};//{ "SHITOMASI", "HARRIS", "FAST", "BRISK", "ORB", "AKAZE", "SIFT"};  
    vector<string> descriptorTypes = { "BRISK", "BRIEF", "ORB"};//{ "BRISK", "BRIEF", "ORB", "FREAK", "AKAZE", "SIFT"};                        
            ...
            for(const auto& matcher_type : matcherTypes)
            {
                for(const auto& selector_type : selectorTypes)
                {
                    ...
                    for( int i=0 ; i < 20; i++)
                    {
                        ...
                        singleType.CameraTTC[i] = 0.0f;
                        singleType.LidarTTC[i] = 0.0f;
                    }
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            //Best match found out
            for (auto &best : bbBestMatches) 
            {
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    oneType.LidarTTC[imgIndex] = ttcLidar;
                    //// EOF STUDENT ASSIGNMENT
                    ...
                    oneType.CameraTTC[imgIndex] = ttcCamera;
    ...
    if (writeFile)
    {
    std::string filename = "../TTC_Table.csv";
    ...
    for( const auto& oneType : CollectionOfTypes)
    {
        for( int i= 0 ; i < 19; i++)
        {
            outputFile << i+1 << "," 
                       ...
                       << std::fixed <<std::setprecision(2) << oneType.LidarTTC[i]<<"s" <<"," 
                       << std::fixed <<std::setprecision(2) << oneType.CameraTTC[i]<<"s" <<","
                       << endl;
        }
        outputFile << endl;
    }
    outputFile.close();
    cout<<"TTC_table.csv is written"<<endl;
    }
    cout<<"Execution completed succesfully"<<endl;
